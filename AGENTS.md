...
- From the repo root, use focused checks for touched files instead, e.g. `git config -f git/.gitconfig --list >/dev/null` and, if the shell is available, `pwsh -NoProfile -Command "[void][scriptblock]::Create((Get-Content -Raw 'shell/Microsoft.PowerShell_profile.ps1'))"` or `zsh -n shell/.zshrc`.
